...
- Relevant files & symbols
  ...
  - `app/vectorstore.py`: `QdrantVectorStore.search()`, `to_source_chunks()`
...
- Detailed walkthrough
  ...
  1. High-level flow
     ...
     - Query path: `answer_question(question)`:
       ...
       3. Retrieve top-k candidates from `QdrantVectorStore.search()` using `settings.top_k`.
...
- Design rationale & tradeoffs
  - Separation of concerns: pipeline only orchestrates embedding, retrieval, prompt assembly, and LLM call — model details remain abstracted behind `EmbeddingService` and `QdrantVectorStore`.
...
- Practical exercises
  ...
  2. Simulate no-retrieval case by using an empty `QdrantVectorStore` and verify the returned canned message.
  3. Create a test that injects a mock `EmbeddingService` and `QdrantVectorStore` to assert `_build_prompt` formatting.
